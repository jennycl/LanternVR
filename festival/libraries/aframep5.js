...
function initializerSettersAndGetters(entity) {
  ...
  entity.setPosition = function(nx, ny, nz) {
    this.x = parseFloat(nx.toFixed(2));
    this.y = parseFloat(ny.toFixed(2));
    this.z = parseFloat(nz.toFixed(2));
  ...
  entity.setY = function(y) {
    var temp = parseFloat(y.toFixed(2));
    if (temp != this.y) {
      this.y = temp;
      this.tag.setAttribute('position', this.x + ' ' + this.y + ' ' + this.z);
    }
  ...
  entity.setZ = function(z) {
    var temp = parseFloat(z.toFixed(2));
    if (temp != this.z) {
      this.z = temp;
      this.tag.setAttribute('position', this.x + ' ' + this.y + ' ' + this.z);
    }
...
function World(id) {
  console.log("A-FrameP5 v0.1 (Craig Kapp, 12/8/2016)");
  ...
  var _interval = setInterval(function() {
    ...
    if (_this.slideMode.enabled) {
      ...
      // execute the slideMode update function
      if ('updateFunction' in _this.slideMode && _this.slideMode.updateFunction != undefined) {
        _this.slideMode.updateFunction();
      }
      ...
      if (_this.slideMode.currentStep >= _this.slideMode.steps) {
        ...
        if ('completeFunction' in _this.slideMode && _this.slideMode.completeFunction != undefined) {
          _this.slideMode.completeFunction();
        }
      }
    }
  ...
  this.slideToObject = function(element, time, updateFunction, completeFunction) {
    ...
    if (this.slideMode.enabled == false) {
      ...
      // store our update & completion function
      this.slideMode.updateFunction = updateFunction;
      this.slideMode.completeFunction = completeFunction;
